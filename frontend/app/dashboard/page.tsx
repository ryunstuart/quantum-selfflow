        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-white/10 bg-black/95 py-8">
            <div className="flex flex-col gap-6 text-center text-lg font-medium">
              ...
              <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="text-red-400 py-2">Logout</button>
            </div>
          </div>
        )}
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 flex-1">
          ...
          <div className="h-80 flex items-end gap-4 md:gap-6 px-4">
            {[42, 71, 88, 76, 105, 138].map((height, i) => (
              <div key={i} className="flex-1 flex flex-col items-center justify-end h-full gap-3 group">
                ...
                <div className="bg-gradient-to-t from-cyan-400 to-emerald-400 w-full rounded-t-xl transition-all duration-700" style={{ height: `${height}%` }} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-8 md:p-10 mb-12">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => alert("✅ Full claims report downloaded!")} className="bg-white/10 hover:bg-white/20 px-6 py-3 rounded-2xl text-sm transition">📥 Download Report</button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <button onClick={() => setShowAddModal(true)} className="bg-cyan-400 hover:bg-cyan-300 active:scale-[0.97] text-slate-950 rounded-3xl p-10 text-center transition-all duration-200 shadow-lg">
          ...
          <button onClick={() => alert("🔗 TPA Integration module coming soon!")} className="bg-slate-900/80 border border-white/10 hover:border-purple-400 active:scale-[0.97] rounded-3xl p-10 text-center transition-all duration-200">
            <div className="text-6xl mb-6">🔗</div>
            <div className="text-2xl font-semibold">Add TPA Extension</div>
